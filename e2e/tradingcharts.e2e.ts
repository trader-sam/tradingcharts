...
test.describe("TradingCharts demo", () => {
  ...
  test("controls and observes a consumer chart viewport", async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      chart.setVisiblePriceRange({ from: 5, to: 25 });
      const fixedPrice = chart.getVisiblePriceRange();
      chart.resetPriceScale();
      const automaticPrice = chart.getVisiblePriceRange();
      unsubscribe();
      chart.setVisibleLogicalRange({ from: 0, to: 1 });
      const interaction = host.querySelector("canvas")?.style.touchAction;
      chart.destroy();
      host.remove();
      return { logical, time, restored, live, fixedPrice, automaticPrice, observed, interaction };
    });
    ...
    expect(result.fixedPrice).toEqual({ from: 5, to: 25 });
    expect(result.automaticPrice.from).not.toBe(5);
