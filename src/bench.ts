...
type Result = { operation:string; tradingchartsMs:number; lightweightChartsMs:number };
...
function addTradingChartsPane(node:HTMLElement,source:Bar[]){const chart=createChart(node);chart.setData(source);chart.addPane({id:'ema',height:.28,title:'EMA 20'});const ema=chart.createSeries({id:'ema',pane:'ema',type:'line',color:'#7dd3fc',lineWidth:2});ema.setData(emaPoints(source));return {chart,ema}}
...
function renderComparisonCharts() {
 ...
 const {chart:open}=addTradingChartsPane(openHost,sample);
 open.fitContent();
